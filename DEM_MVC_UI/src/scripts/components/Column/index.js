...
class Column extends Component {
    ...
    constructClassNames = (sizesArray, classNames) =>(
        sizesArray.filter(size => size.count)
            .map((size) => `col-${ size.name }-${ size.count }`)
            .reduce((previouse, current) => `${ previouse } ${ classNames[ current ] }`, ''));

    render(){
        const {xs, sm, md, lg, xl, xsOffset, smOffset, mdOffset, lgOffset, xlOffset, children, classNames } = this.props;
        const classes = this.constructClassNames([
            { name: 'xs', count: xs },
            { name: 'sm', count: sm },
            { name: 'md', count: md },
            { name: 'lg', count: lg },
